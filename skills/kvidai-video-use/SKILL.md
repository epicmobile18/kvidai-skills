...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached raw transcript JSON (whisperx)
...
- `yt-dlp`, HyperFrames, Manim installed only on first use.
- For kvidai handoff: `KVIDAI_API_KEY` env var and the sibling skill at `../kvidai-video-project/scripts/kvidai-client.mjs` (Node.js 20+). No separate install — use `node kvidai-client.mjs` directly.
- First-use animation setup happens inside the slot directory, never at the video-use repo root. HyperFrames can be invoked with `npx --yes hyperframes ...`.
...
- **`transcribe.py <video>`** — single-file transcription (whisperx). `--num-speakers N` optional. Cached.
...
Pick the engine per animation slot.
...
5. Run kvidai AI agent — pass transcript via --cdn-url for free subtitle generation
   ...
   # Without transcript: omit --cdn-url; agent will notify that no transcript is available (Phase 2: auto-STT).
...
- **Running Whisper locally on CPU.** Slow and it normalizes fillers. Use `whisperx` with GPU acceleration when available.
