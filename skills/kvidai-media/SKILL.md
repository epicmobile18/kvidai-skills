...
The APIM policy injects the subscription's owner email as `X-Kvidai-User-Email`, so the upload is auto-scoped to that user — no need to pass an email in the body.
